...
if __name__ == "__main__":
    ...
    while num <= model_num:
        ...
        if sys.platform.startswith("darwin"):
            os.system("python3 CNN_therm_dev.py")
        else:
            os.system("python CNN_therm_dev.py")
        ...
        if (
            test_dict_basic["macro avg"]["precision"] >= threshold_dumb
            and test_dict_basic["macro avg"]["recall"] >= threshold_dumb
            and test_dict_basic_2["macro avg"]["precision"] >= threshold_dumb
            and test_dict_basic_2["macro avg"]["recall"] >= threshold_dumb
            and test_dict_basic_3["macro avg"]["precision"] >= threshold_dumb
            and test_dict_basic_3["macro avg"]["recall"] >= threshold_dumb
            and test_dict_basic_4["macro avg"]["precision"] >= threshold_dumb
            and test_dict_basic_4["macro avg"]["recall"] >= threshold_dumb
        ):
            ...
            # ):
            if (
                test_dict_disord["macro avg"]["precision"] > threshold_smart
                and test_dict_disord["macro avg"]["recall"] > threshold_smart
            ):
                smart_num += 1
                if smart_num <= smart_count:
                    num += 1
                # Model renaming
                old_name = "Models/upgraded_model_x_therm.dict"
                new_name = f"Models/smart_Models/smart_model_therm_autogenerate_{smart_num}.dict"
                os.rename(old_name, new_name)

                # History file renaming
                old_name_hist = "Models/upgraded_model_x_therm_history.pickle"
                new_name_hist = f"Models/smart_Models/smart_model_therm_autogenerate_{smart_num}_history.pickle"
                ...
                old_name = "Models/upgraded_model_x_therm.dict"
                new_name = (
                    f"Models/dumb_Models/dumb_model_therm_autogenerate_{dumb_num}.dict"
                )
                os.rename(old_name, new_name)

                # History file renaming
                old_name_hist = "Models/upgraded_model_x_therm_history.pickle"
                new_name_hist = f"Models/dumb_Models/dumb_model_therm_autogenerate_{dumb_num}_history.pickle"
                os.rename(old_name_hist, new_name_hist)
